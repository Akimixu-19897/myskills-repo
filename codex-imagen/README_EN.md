...
```text
Codex, install this Skill for me: https://github.com/Akimixu-19897/myskills-repo/tree/main/codex-imagen
```

Use it:

```text
$codex-imagen Generate a 16:9 4K landscape image and show me the result
```
...
Codex Imagen is a Codex Skill for generating and editing images through an OpenAI-compatible relay or proxy endpoint, with saved prompts and non-secret run metadata.
...
Codex Imagen wraps Codex's bundled image generation CLI:
...
```powershell
git clone https://github.com/Akimixu-19897/myskills-repo/tree/main/codex-imagen.git "$HOME/.codex/skills/codex-imagen"
```

On Windows, the target directory is usually:

```text
C:\Users\<you>\.codex\skills\codex-imagen
```

After installation, Codex can use the Skill when you mention Codex Imagen image generation, saved prompts, run metadata, ccswitch, `api_key.json`, or `$codex-imagen`.
...
```powershell
$setup = "$HOME/.codex/skills/codex-imagen/scripts/setup.py"
python $setup --check
```
...
Codex Imagen supports three configuration sources:
...
4. `CODEX_IMAGEN_CONFIG`
5. `photo/api_key.json` under the current project
6. `.secrets/image_api.json` under the current project
7. `.secrets/codex_imagen.json` under the current project
8. `%APPDATA%/codex-imagen/config.json` on Windows
9. `~/.config/codex-imagen/config.json`
10. `~/.codex-imagen.json`
...
```powershell
python ~/.codex/skills/codex-imagen/scripts/setup.py --check-codex
```
...
Note: ccswitch may store multiple endpoint URLs. The last endpoint is not necessarily the active or healthy one. Codex Imagen no longer picks the last endpoint blindly, which avoids accidentally selecting a failed node.
...
```powershell
python ~/.codex/skills/codex-imagen/scripts/setup.py --check-ccswitch
```
...
```powershell
python ~/.codex/skills/codex-imagen/scripts/setup.py config --scope user
```
...
```powershell
python ~/.codex/skills/codex-imagen/scripts/setup.py config --scope skill
```

Path:

```text
~/.codex/skills/codex-imagen/.secrets/config.json
```
...
```powershell
python ~/.codex/skills/codex-imagen/scripts/setup.py --check
```
