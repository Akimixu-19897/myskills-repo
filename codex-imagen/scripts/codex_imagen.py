...
def normalize_proxy_url(raw_url: str) -> str:
    ...
    try:
        ...
        die(f"Invalid relay proxy URL: {exc}")
    if scheme not in {"http", "https", "socks5", "socks5h"}:
        die("Relay proxy must use http, https, socks5, or socks5h.")
    if not hostname or port is None:
        die("Relay proxy URL must include a host and port.")
    return proxy_url
...
def configure_proxy(env: dict[str, str]) -> tuple[str, str]:
    ...
    try:
        ...
        die(
            f"Clash proxy is unavailable at {safe_proxy_url(proxy_url)} ({exc}). "
            "Start Clash or set CODEX_IMAGEN_PROXY_URL to its proxy endpoint."
        )

    # Set every common proxy variable and remove bypass lists so the child
    # image client cannot silently fall back to a direct connection.
    for name in ("ALL_PROXY", "all_proxy", "HTTP_PROXY", "http_proxy", "HTTPS_PROXY", "https_proxy"):
        env[name] = proxy_url
    env["NO_PROXY"] = ""
    env["no_proxy"] = ""
    return proxy_url, safe_proxy_url(proxy_url)


def default_config_path(include_skill_private: bool = True) -> Path | None:
    env_path = os.environ.get("CODEX_IMAGEN_CONFIG")
    candidates = []
    if env_path:
    ...
    candidates.extend(
        [
            ...
            Path.cwd() / ".secrets" / "codex_imagen.json",
            SKILL_PRIVATE_CONFIG if include_skill_private else None,
            Path(appdata) / "codex-imagen" / "config.json" if appdata else None,
            Path.home() / ".config" / "codex-imagen" / "config.json",
            Path.home() / ".codex-imagen.json",
        ]
    )
...
def load_config(
    ...
    if from_codex or os.environ.get("CODEX_IMAGEN_FROM_CODEX") == "1":
        return load_codex_config(codex_config, codex_auth)
    use_codex = not no_codex and os.environ.get("CODEX_IMAGEN_NO_CODEX") != "1"
    use_ccswitch = not no_ccswitch and os.environ.get("CODEX_IMAGEN_NO_CCSWITCH") != "1"
    if from_ccswitch or os.environ.get("CODEX_IMAGEN_FROM_CCSWITCH") == "1":
...
def default_output_path(args: argparse.Namespace) -> Path:
    out_dir = Path(args.output_dir or os.environ.get("CODEX_IMAGEN_OUTPUT_DIR", DEFAULT_OUT_DIR))
    if args.name:
        base = args.name
...
def write_sidecar(
    ...
    meta = {
        ...
        "proxy_url": safe_proxy_url(proxy_url),
...
def main() -> int:
    ...
    temp_context = (
        ...
        else tempfile.TemporaryDirectory(prefix="codex-imagen-")
    )
    with temp_context as temp_dir:
        ...
        env["CODEX_IMAGEN_USER_AGENT"] = str(cfg.get("user_agent") or "Mozilla/5.0")
        proxy_url, proxy_display = configure_proxy(env)
        runtime_path = str(SKILL_DIR / "scripts")
        existing_pythonpath = env.get("PYTHONPATH")
        env["PYTHONPATH"] = (
            runtime_path
            if not existing_pythonpath
            else f"{runtime_path}{os.pathsep}{existing_pythonpath}"
        )
        ...
        write_sidecar(out, args, cfg, config_path, images, dimensions, time.time() - started, proxy_url)
        return 0
