...
Use `scripts/codex_imagen.py` directly. The defaults are tuned for low-thinking relay image runs:
...
- High quality.
- Requests always route through the local Clash SOCKS5 proxy `127.0.0.1:7897`; the wrapper fails fast if the proxy is unavailable and never silently falls back to a direct connection.
...
```powershell
$skill = "$HOME/.codex/skills/codex-imagen/scripts/codex_imagen.py"
```
...
Use `scripts/codex_imagen.py` for Codex Imagen image generation or editing. It reads a private JSON config only at runtime, injects `OPENAI_API_KEY` and `OPENAI_BASE_URL` only into the child process that calls the bundled imagegen CLI, then verifies output dimensions when possible.

## Clash Proxy

The wrapper routes the bundled image client through `socks5h://127.0.0.1:7897` for every real request. It checks that the proxy port is reachable, sets `ALL_PROXY`, `HTTP_PROXY`, and `HTTPS_PROXY` (including lowercase variants), and clears `NO_PROXY` so external relay requests cannot bypass Clash.

To use a different local Clash endpoint, set `CODEX_IMAGEN_PROXY_URL` or `CLASH_PROXY_URL` to an explicit `http://`, `https://`, `socks5://`, or `socks5h://` URL. There is no direct-connection fallback:

```bash
CODEX_IMAGEN_PROXY_URL=socks5h://127.0.0.1:7897 \
  python scripts/codex_imagen.py generate --prompt-file prompts/prompt.txt --name output --force
```
...
5. `CODEX_IMAGEN_CONFIG` if set.
6. `photo/api_key.json` under the current working directory.
7. `.secrets/image_api.json` under the current working directory.
8. `.secrets/codex_imagen.json` under the current working directory.
9. `%APPDATA%/codex-imagen/config.json` on Windows.
10. `~/.config/codex-imagen/config.json`.
11. `~/.codex-imagen.json`.
...
```text
<skill>/codex-imagen/.secrets/config.json
```

Use the setup helper when available:

```powershell
$skillDir = "$HOME/.codex/skills/codex-imagen"
...
%APPDATA%/codex-imagen/config.json
~/.config/codex-imagen/config.json
```
...
If `--out` is omitted, the script writes a timestamped file under `--output-dir`, `CODEX_IMAGEN_OUTPUT_DIR`, or finally `generated`. For example: `generated/output-2k-20260526-203000-2k.png`.
...
- `--output-dir <path>`: Directory for auto-named outputs when `--out` is omitted. Default is `generated`, or `CODEX_IMAGEN_OUTPUT_DIR` if set.
...
- Proxy endpoint (host and port only; no credentials).
