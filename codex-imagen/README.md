...
```text
Codex，帮我安装一下这个 Skill：https://github.com/Akimixu-19897/myskills-repo/tree/main/codex-imagen
```

使用：

```
$codex-imagen 跑一张 16:9、4K 的风景图看看效果
```
...
Codex Imagen 是一个 Codex Skill，用于通过兼容 OpenAI 接口的中转站或代理生成、编辑图片，并自动保存提示词和不含密钥的出图记录。
...
Codex Imagen 当前是 Codex 自带图片生成脚本的包装层：
...
```text
~/.codex/skills/codex-imagen
```

Windows 通常为：

```text
C:\Users\<你的用户名>\.codex\skills\codex-imagen
```
...
安装后，当你在 Codex 中提到中转站出图、保存提示词、出图记录、ccswitch、`api_key.json` 或 `$codex-imagen` 时，Codex 就可以使用这个 Skill。
...
4. 环境变量 `CODEX_IMAGEN_CONFIG` 指向的 JSON 文件
5. 当前项目下的 `photo/api_key.json`
6. 当前项目下的 `.secrets/image_api.json`
7. 当前项目下的 `.secrets/codex_imagen.json`
8. Windows 下的 `%APPDATA%/codex-imagen/config.json`
...
注意：ccswitch 的 endpoint 列表可能包含多个节点，最后一个节点不一定是当前可用节点。Codex Imagen 不会再简单读取最后一个 endpoint，以免误选到失败节点。
